...
test('keyboard resize: resize() forces a full Ace repaint and re-measures the container', async ({ page }) => {
  // Regression guard for the Android soft-keyboard render bug. When the
  // soft keyboard opens/closes, `visualViewport.height` changes but the
  // *window* never fires 'resize' on Android Chrome. sterk's
  // AceRenderer.resize() only re-syncs buffer→document; it never calls
  // editor.resize(), so Ace can keep painting against the stale viewport
  // height → frozen/garbled lower rows with glyph overdraw. The fix makes
  // core.resize() explicitly call editor.resize(true) + renderer.updateFull()
  // (see terminal-core.js `redraw()`), which re-measures the container and
  // does a full repaint that clears stale glyphs.
  //
  // Headless chromium has a working ResizeObserver on Ace's container, so
  // it cannot reproduce the *visual* artifact (Ace re-measures on its own
  // here). We therefore guard the fix two ways:
  //   1. assert resize() actually invokes the full-repaint path
  //      (editor.resize + renderer.updateFull) — the wiring that is missing
  //      on the real device, and
  //   2. assert Ace's cached viewport height tracks the container after a
  //      shrink→grow cycle (no stale dimensions).
  const captured = seedErrorCapture(page);
  await bootTerminal(page);

  // (1) Spy on the Ace editor's full-repaint methods, then run resize().
  const calls = await page.evaluate(() => {
    const ed = window.__mobuxView.test.aceEditor();
    if (!ed) return null;
    const counts = { resize: 0, updateFull: 0 };
    const origResize = ed.resize.bind(ed);
    ed.resize = (force) => { counts.resize++; return origResize(force); };
    const r = ed.renderer;
    const origFull = r.updateFull.bind(r);
    r.updateFull = (force) => { counts.updateFull++; return origFull(force); };
    window.__mobuxView.test.triggerResize();
    return counts;
  });
  expect(calls, 'must reach the real Ace editor').not.toBeNull();
  expect(calls.resize, 'resize() must force Ace to re-measure (editor.resize)').toBeGreaterThan(0);
  expect(calls.updateFull, 'resize() must force a full repaint (renderer.updateFull)').toBeGreaterThan(0);

  // (2) Shrink the body (= .term-body, height:100vh) like input-bar.js does
  // for the keyboard, so the flex child #terminal shrinks. Then grow back.
  const initialAceH = await page.evaluate(() => window.__mobuxView.test.aceViewportHeight());
  expect(initialAceH, 'Ace cached viewport height').toBeGreaterThan(0);

  const shrunk = await page.evaluate(async () => {
    document.body.style.height = `${Math.round(window.innerHeight * 0.55)}px`;
    document.getElementById('terminal').getBoundingClientRect(); // flush layout
    window.__mobuxView.test.triggerResize();
    await new Promise((r) => requestAnimationFrame(() => r()));
    return {
      hostH: Math.round(document.getElementById('terminal').getBoundingClientRect().height),
      aceH: window.__mobuxView.test.aceViewportHeight(),
    };
  });
  expect(shrunk.aceH, 'Ace re-measures down with the container').toBeLessThan(initialAceH);
  expect(Math.abs(shrunk.aceH - shrunk.hostH), 'Ace height tracks shrunk container').toBeLessThan(40);

  const grown = await page.evaluate(async () => {
    document.body.style.height = '';
    document.getElementById('terminal').getBoundingClientRect();
    window.__mobuxView.test.triggerResize();
    await new Promise((r) => requestAnimationFrame(() => r()));
    return {
      hostH: Math.round(document.getElementById('terminal').getBoundingClientRect().height),
  ...
  expect(grown.aceH, 'Ace re-measures back up with the container').toBeGreaterThan(shrunk.aceH);
  expect(Math.abs(grown.aceH - grown.hostH), 'Ace height tracks grown container').toBeLessThan(40);
