...
test('focus snap: viewport snaps to bottom when terminal input gains focus', async ({ page }, testInfo) => {
  // Regression for issue #99. When the user is parked mid-scrollback and
  // taps the terminal to type, the soft keyboard comes up but the
  // viewport stays in scrollback — so what they type lands somewhere
  // they can't see. Renderer-agnostic page-level focusin handler in
  // terminal.js snaps to the live screen on focus.
  //
  // Originally proposed as sterk PR #47 (closed): policy-in-renderer
  // produces divergent UX across the dual backend. Moved to mobux to
  // keep both consistent.
  //
  // Setup uses the existing `__mobuxView.test.injectLines()` helper
  // (closes the WS first so tmux can't clobber the injected content)
  // rather than typing via the live PTY. Both backends grow their
  // scrollback the same way through their VT parsers on a synthetic
  // newline-rich write — what we're testing is the page-level focus
  // handler, not tmux's redraw protocol.
  const captured = seedErrorCapture(page);
  await bootTerminal(page);
  ...
  // Inject rows + 20 lines via the synthetic helper. Both backends'
  // parsers see LF as "newline at bottom row → scrollback grows".
  const totalLines = rows + 20;
  const lastIdx = totalLines - 1;
  const marker = `FOCUS_SNAP_${Math.floor(Math.random() * 1e9)}`;
  await page.evaluate(({ n, m }) => window.__mobuxView.test.injectLines(n, m), { n: totalLines, m: marker });
  // Give the writes a beat to flush through the parser / renderer.
  ...
  // Scroll up by 5 rows so the last injected line is well off-screen.
  await page.evaluate(() => {
  ...
  // Pre-condition: viewportY moved off bottom. If the renderer didn't
  // accumulate scrollback (e.g. test env can't fill the buffer), bail
  // with a useful message rather than false-positive.
  const preViewportY = await page.evaluate(() => window.__mobuxView.test.viewportY());
  expect(
    preViewportY,
    `pre-condition: viewportY (${preViewportY}) should be less than bottomViewportY (${bottomViewportY}) after scrollLines(-5)`,
  ).toBeLessThan(bottomViewportY);

  // Trigger focus on the terminal's hidden textarea — covers both
  // backends (xterm: `.xterm-helper-textarea`, sterk: `.ace_text-input`).
  const focused = await page.evaluate(() => {
    const ta = document.querySelector(
      '#terminal .xterm-helper-textarea, #terminal .ace_text-input, #terminal textarea',
    );
    if (!ta) return false;
    ta.focus();
    return document.activeElement === ta;
  });
  expect(focused, 'must be able to focus terminal textarea').toBe(true);

  // Give the focusin handler a beat to fire + the render to land.
  await page.waitForTimeout(150);

  // Artifact screenshot for debugging.
  const screenshotPath = `.tmp/focus-snap-${testInfo.project.name}.png`;
  await page.screenshot({ path: screenshotPath, fullPage: false });

  // Post-condition: viewportY snapped back to bottom AND the last
  // injected line's marker text is rendered inside #terminal's box.
  const postViewportY = await page.evaluate(() => window.__mobuxView.test.viewportY());
  expect(
    postViewportY,
    `viewportY should snap back to ${bottomViewportY} after focus, got ${postViewportY}; screenshot: ${screenshotPath}`,
  ).toBe(bottomViewportY);

  // Last injected line: "<marker> <lastIdx>" should be in the rendered
  // DOM and inside the terminal's bounding box.
  const lastLine = `${marker} ${lastIdx}`;
  const markerGeom = await page.evaluate((m) => {
    const t = document.getElementById('terminal');
    const tRect = t.getBoundingClientRect();
    const walker = document.createTreeWalker(t, NodeFilter.SHOW_TEXT);
    ...
    while ((node = walker.nextNode())) {
      if (node.data && node.data.includes(m)) {
        ...
        const rect = r.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0) {
          best = { top: rect.top, bottom: rect.bottom };
        }
      }
    }
    return { best, tRect: { top: tRect.top, bottom: tRect.bottom } };
  }, lastLine);

  expect(
    markerGeom.best,
    `last line "${lastLine}" must be rendered after snap-to-bottom; screenshot: ${screenshotPath}`,
  ).toBeTruthy();
  expect(
    markerGeom.best.bottom,
    `last-line bottom (${markerGeom.best?.bottom}) should be within #terminal bottom (${markerGeom.tRect.bottom}); screenshot: ${screenshotPath}`,
  ).toBeLessThanOrEqual(markerGeom.tRect.bottom + 4);
  expect(
    markerGeom.best.top,
    `last-line top (${markerGeom.best?.top}) should be within #terminal top (${markerGeom.tRect.top}); screenshot: ${screenshotPath}`,
  ).toBeGreaterThanOrEqual(markerGeom.tRect.top - 4);
